...
// 路由权限白名单
const allowList = ['adminLogin', 'login', 'adminRegister', 'portal', 'search', 'detail', '403', '404']
// 前台登录地址
const loginRoutePath = '/index/login'
// 后台登录地址
const adminLoginRoutePath = '/adminLogin'
// 后台登录地址
const adminRegisterRoutePath = '/adminRegister'
...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.path.startsWith('/admin')) {
    if (localStorage.getItem(ADMIN_USER_TOKEN)) {
      if (to.path === adminLoginRoutePath || to.path === adminRegisterRoutePath) {
        next({ path: '/' })
      } else {
        next()
      }
